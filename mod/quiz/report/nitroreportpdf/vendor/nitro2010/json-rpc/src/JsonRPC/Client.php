...
class Client
{
    ...
    private function doRequest(array $payload)
    {
		$ch=curl_init();
		if($ch === false):
			throw new Exception('No cURL initialisation');
		endif;
		curl_setopt($ch,CURLOPT_URL,trim($this->url));
		...
		$ret=curl_exec($ch);
		if($ret === false):
			throw new ConnectionFailureException('curl_exec() returned false .. message: '.curl_error($ch));
		endif;
		curl_close($ch);	
